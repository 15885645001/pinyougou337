...
app.controller('goodsController' ,function($scope,$controller,$location,typeTemplateService ,itemCatService,uploadService ,goodsService){	
	...
	$scope.findOne=function(){	
		var id = $location.search()['id'];//取出id
		...
		//点击修改按钮过来的  id应该回显
		goodsService.findOne(id).success( //通过商品id查询商品包装对象(商品对象 商品详情对象 库存集对象)
			function(response){
	...
	$scope.selectItemCat1List = function(){
		//商品分类服务层调用findByParentId(0)方法 查询所有商品分类(即父ID为0的)
		itemCatService.findByParentId(0).success(function(response){ //response代表的是List<ItemCat>
	...
	$scope.$watch("entity.goods.typeTemplateId",function(newValue,oldValue){
		...
		typeTemplateService.findOne(newValue).success(function(response){
			...
			// 将品牌的字符串数据转成JSON
			$scope.typeTemplate.brandIds = JSON.parse( $scope.typeTemplate.brandIds );//将json串转为对象,才可以进行遍历
	...
	$scope.findItemCatList = function(){
		itemCatService.findAll().success(function(response){ //response 代表 List<ItemCat>商品分类结果集
